// https://observablehq.com/@juarezmeneses/homicidios-em-fortaleza-em-2012@129
  ...
  main.variable(observer("geojson")).define("geojson", ["L","info","map","bairros","style"], function(L,info,map,bairros,style)
{
  ...
  main.variable(observer("legend")).define("legend", ["L","schemeOranges","colorScale","d3","map"], function(L,schemeOranges,colorScale,d3,map)
{
  let legendControl = L.control({position: 'bottomright'});
	...
	legendControl.onAdd = function (map) {
		...
		let div = L.DomUtil.create('div', 'info legend'),
			labels = [],
            n = schemeOranges.length,
			from, to;

		for (let i = 0; i < n; i++) {
			let c = schemeOranges[i]
            let fromto = colorScale.invertExtent(c);
			labels.push(
				'<i style="background:' + schemeOranges[i] + '"></i> ' +
...
<main role="main" class="container">
 ...
 `
  ...
  main.variable(observer("schemeOranges")).define("schemeOranges", ["d3"], function(d3){return(
d3.schemeOranges[9]
)});
  main.variable(observer("colorScale")).define("colorScale", ["d3","schemeOranges"], function(d3,schemeOranges){return(
d3.scaleQuantize()
    .domain([0, 70])
    .range(schemeOranges)
)});
...
function zoomToFeature(e) {
  ...
  main.variable(observer("info")).define("info", ["L","crimesPopulationByName","map"], function(L,crimesPopulationByName,map)
{
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Taxa de homicídios</h5>' +  (feat ?
				'<b>' + feat.properties.NOME + '</b><br />'
              + crimesPopulationByName.get(feat.properties.NOME)[0] + ' homicídios' + '<br/>'        
              + crimesPopulationByName.get(feat.properties.NOME)[2] + ' população' + '<br/>'  
              + crimesPopulationByName.get(feat.properties.NOME)[1] + ' taxa de homicídios por 100 mil habitantes'
  ...
  main.variable(observer("crimesPopulationByName")).define("crimesPopulationByName", ["d3"], function(d3){return(
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data) {
    ...
    data.forEach(function(d, i) {
      d.rateHomicidesByPopulation = ((d.Homicidios/d.Populacao)*100000).toFixed(2)
      bairrosMap.set(d.Bairro, [+d.Homicidios, d.rateHomicidesByPopulation, d.Populacao])
    })
    return bairrosMap
  })
)});
...
d3.csv("https://gist.githubusercontent.com/emanueles/bae10fc42b13886b5a00b79737f2b50d/raw/ffb35abe6d9ba0c4ce4866b827f1916748ab65ef/homicidios_2012.csv").then(function(data) {
  ...
  main.variable(observer("style")).define("style", ["colorScale","crimesPopulationByName"], function(colorScale,crimesPopulationByName){return(
function style(feature) {
		 return {
					...
					dashArray: '3',
					fillOpacity: 0.6,
					fillColor: colorScale(crimesPopulationByName.get(feature.properties.NOME)[1])
				};
	}
)});
